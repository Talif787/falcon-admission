...
export default function InterviewPage() {
  ...
  const initializeChat = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/chat/${sessionId}/init`);
      ...
      if (data.success) {
        ...
        // Check if program selection is needed
        if (data.data.message.toLowerCase().includes('business') && 
  ...
  const sendMessage = async (messageText?: string, selectedProgram?: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/chat/${sessionId}/message`, {
        method: 'POST',
        headers: {
      ...
      <div className="max-w-4xl mx-auto p-4 h-[calc(100vh-80px)] flex flex-col">
        ...
        {showProgramSelect && !isCompleted && (
          <div className="mb-4 p-4 bg-white rounded-lg shadow-sm border border-blue-200">
            <p className="text-sm font-medium text-gray-900 mb-3">Please select your program of interest:</p>
            <div className="flex gap-3">
              <button
